...
An X-Correlation-ID HTTP header must be included into each message sent to the aggregating application by an authority. Also data suppliers who use the data disclosure system must give an X-Correlation-ID to the response delivered to the data disclosure system. The identifier is reported in UUIDv4 form. The application uses the identifier to link all messages that are related to the same query. The X-Correlation-ID must be same for each message that is related to the the same query. An X-Correlation-ID is given to a query when a new query is made to the Query API. After that the same X-Correlation-ID is used in all Status and Result API messages.
